...
class _OnboardingStep3State extends State<OnboardingStep3> {
  ...
  final Set<String> _selected = {'Food', 'Shopping', 'Transport', 'Entertainment', 'Other'};
    ...
    try {
      ...
      if (wallets.isNotEmpty) {
        ...
        for (final catCode in _selected) {
          final text = _controllers[catCode]?.text.replaceAll('.', '').replaceAll(',', '').trim() ?? '';
    ...
    return Scaffold(
      body: Container(
        ...
        child: SafeArea(
          child: Column(
            children: [
              _ProgressHeader(label: 'Bước 3/4', percent: '75%', value: 0.75,
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Chọn danh mục cần đặt giới hạn:', style: Theme.of(context).textTheme.bodySmall?.copyWith(fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                            ...
                            return GestureDetector(
                              ...
                              child: AnimatedContainer(
                                ...
                                decoration: BoxDecoration(
                                  color: isOn ? cat.color.withValues(alpha: 0.12) : const Color(0xFFF8FAFC),
                                  borderRadius: BorderRadius.circular(AppRadii.lg),
                                  border: Border.all(color: isOn ? cat.color : AppColors.border, width: isOn ? 1.5 : 1),
                                ),
                                child: Row(mainAxisSize: MainAxisSize.min, children: [
                                  CategoryTheme.iconOf(cat.code, size: 16),
                                  const SizedBox(width: 5),
                                  Text(cat.label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: isOn ? cat.color : AppColors.textPrimary)),
                                ]),
                              ),
                            );
                          }).toList(),
                        ...
                        ..._allCats.where((c) => _selected.contains(c.code)).map((cat) => Padding(
                          padding: const EdgeInsets.only(bottom: 12),
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Row(children: [
                              Container(width: 28, height: 28, decoration: BoxDecoration(color: cat.color.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(8)),
                                  child: Center(child: CategoryTheme.iconOf(cat.code, size: 16))),
                              const SizedBox(width: 8),
                              Text(cat.label, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
                            ]),
                            const SizedBox(height: 6),
                            TextField(controller: _controllers[cat.code], keyboardType: TextInputType.number,
...
class _ProgressHeader extends StatelessWidget {
  ...
  const _ProgressHeader({required this.label, required this.percent, required this.value, this.onSkip});
