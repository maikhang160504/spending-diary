...
class _ShareWalletScreenState extends State<ShareWalletScreen> {
        ...
        builder: (ctx, setDialogState) {
          ...
          if (generatingCode && inviteCode == null && inviteError == null) {
            WidgetsBinding.instance.addPostFrameCallback((_) async {
              try {
                final res = await _api.generateWalletInviteCode(
                  widget.walletId!,
                );
                if (ctx.mounted) {
                  setDialogState(() {
  ...
  Future<void> _confirmLeave() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Bạn có chắc muốn rời khỏi ví nhóm này? Bạn sẽ không còn xem được các giao dịch trong nhóm.',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      MimoSnackBar.showInfo(context, message: 'Đã rời nhóm');
    } on ApiException catch (e) {
      if (!mounted) return;
      MimoSnackBar.showInfo(context, message: e.localizedMessage);
    } catch (_) {
      if (!mounted) return;
      MimoSnackBar.showInfo(
        context,
        message: 'Không thể rời nhóm, vui lòng thử lại.',
      );
    }
  }
  ...
  Future<void> _confirmRemove(Map<String, dynamic> member) async {
    ...
    try {
      ...
      MimoSnackBar.showInfo(context, message: 'Đã xoá thành viên');
    } on ApiException catch (e) {
      if (!mounted) return;
      MimoSnackBar.showInfo(context, message: e.localizedMessage);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                          ...
                          child: Padding(
                            ...
                            child: Center(
                              child: EmptyState(
                                ...
                                subtitle:
                                    'Ghi nhận giao dịch chung đầu tiên ngay!',
                              ),
                            ),
                          ),
                        )
                      : SliverList(
                          delegate: SliverChildBuilderDelegate((ctx, i) {
                            ...
                            return TransactionStoryCard(
                              ...
                              walletOwnerId:
                                  _wallet['ownerId'] ?? _wallet['owner_id'],
                            );
                            ...
                            delegate: SliverChildBuilderDelegate(
                              (ctx, i) {
                                ...
                                return StoryGalleryCard(
                                  story:
                                      galleryStories[i] as Map<String, dynamic>,
                          ...
                          child: ListView(
                            children: [
                              ..._members.map((m) {
                                ...
                                final isCurrentUserOwner = _members.any(
                                  (m) =>
                                      m['id'] == _currentUserId &&
                                      m['role'] == 'owner',
                                );
      ...
      floatingActionButton: RadialMenuFab(
        ...
        onSelectChat: () {
          context.push(AppRoutes.chat, extra: {'walletId': widget.walletId});
        },
        onSelectBill: () {
          context.push(
            AppRoutes.camera,
            extra: {'walletId': widget.walletId, 'initialMode': 'Bill'},
          );
        },
        onSelectPhoto: () {
          context.push(
            AppRoutes.camera,
            extra: {'walletId': widget.walletId, 'initialMode': 'Ảnh'},
          );
        },
        onSelectReport: () {
  ...
  Widget _buildHeader() {
    ...
    final walletColorHex = (_wallet['color'] as String? ?? '#0D9488')
        .replaceAll('#', '');
    Color walletColor;
    try {
      walletColor = Color(
        int.parse(
          walletColorHex.length == 6 ? 'FF$walletColorHex' : walletColorHex,
          radix: 16,
        ),
      );
    } catch (_) {
    ...
    final remaining =
        (_overview['remaining'] as num?)?.toInt() ??
        (_wallet['balance'] as num?)?.toInt() ??
        0;
    return Container(
      decoration: BoxDecoration(
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Center(
                  child: Text(walletIcon, style: const TextStyle(fontSize: 18)),
                ),
              ),
              const SizedBox(width: 10),
              Expanded(
