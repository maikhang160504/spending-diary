...
class GoalScreen extends StatefulWidget {
  ...
  const GoalScreen({super.key, this.isChallenge = false, this.initialJoinCode});
                        ...
                        : () async {
                            ...
                            try {
                              ...
                              if (mounted) {
                                MimoSnackBar.showError(
                                  context,
                                  message: 'Không thể tạo mục tiêu mới',
                                );
                              }
                            }
                          },
                            ...
                            try {
                              ...
                              if (mounted) {
                                MimoSnackBar.showSuccess(
                                  context,
                                  message:
                                      'Đã nạp ${formatVnd(amount.toInt())} vào "$goalName" thành công!',
                                  emotion: 'Celebrate',
                                );
                              }
                            } catch (e) {
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(
                  widget.isChallenge
                      ? 'Nhập mã tham gia thử thách'
                      : 'Nhập mã tham gia nhóm tiết kiệm',
                  style: Theme.of(ctx).textTheme.titleMedium?.copyWith(
                    fontWeight: FontWeight.w700,
                  ),
                ),
                                ...
                                SnackBar(
                                  content: Text(
                                    widget.isChallenge
                                        ...
                                        : '🎉 Tham gia nhóm tiết kiệm thành công!',
                                  ),
                                  backgroundColor: AppColors.teal,
                                ),
                              );
  ...
  Future<void> _showInviteGoal(String goalId, String goalName) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        MimoSnackBar.showInfo(context, message: 'Lỗi tạo mã mời: $e');
      }
      return;
    }
    if (!mounted) return;
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        ...
                        MimoSnackBar.showInfo(
                          context,
                          message: 'Đã sao chép mã mời thử thách!',
                        );
                      },
                    ),
                  ],
                  ...
                  children: [
                    ...
                    InkWell(
                      onTap: () {
                        ...
                        MimoSnackBar.showInfo(
                          context,
                          message: 'Đã sao chép liên kết tham gia!',
                        );
                      },
                      child: const Text(
                        'Copy link',
...
class _ApiGoalCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isChallenge =
        goal['type']?.toString().startsWith('challenge') == true;
    final targetAmountVal = goal['target_amount'] ?? goal['targetAmount'];
    final memberAmountVal =
        goal['member_current_amount'] ?? goal['memberCurrentAmount'];
    ...
    final targetAmount =
        num.tryParse(targetAmountVal?.toString() ?? '')?.toInt() ?? 0;
    final currentAmount =
        num.tryParse(currentAmountVal?.toString() ?? '')?.toInt() ?? 0;
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 3),
                        Text(
                          formatVnd(remaining > 0 ? remaining : 0),
