...
class _GoalDetailScreenState extends State<GoalDetailScreen> {
  ...
  Future<void> _loadGoalDetail() async {
    ...
    try {
      ...
      setState(() {
        ...
        _contributorLeaderboard =
            res['contributorLeaderboard'] as List<dynamic>? ?? [];
        _progressLeaderboard =
            res['progressLeaderboard'] as List<dynamic>? ?? [];
        _myProgress = res['myProgress'] as Map<String, dynamic>?;

        final meResult = resList[2] as Map<String, dynamic>;
        _currentUserId =
            meResult['user']?['id'] as String? ?? meResult['id'] as String?;
  ...
  Future<void> _deleteGoal() async {
    ...
    try {
      if (isOwner) {
        ...
        if (mounted) {
          MimoSnackBar.showInfo(context, message: 'Đã xóa mục tiêu thành công');
          context.pop(true);
        }
      } else {
        await _api.leaveGoal(widget.goalId);
        if (mounted) {
          MimoSnackBar.showInfo(context, message: 'Đã rời khỏi mục tiêu');
          context.pop(true);
        }
      }
    } catch (_) {
      if (mounted) {
        MimoSnackBar.showInfo(
          context,
          message: 'Không thể $actionText mục tiêu',
        );
      }
    }
  }
  ...
  void _showContributeSheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) {
        final isChallenge =
            _goal['type']?.toString().startsWith('challenge') == true;
        final targetAmountVal =
            (_goal['target_amount'] as num?)?.toDouble() ?? 0;
        final isGroupGoal =
            _goal['is_group'] == true || _goal['isGroup'] == true;
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: FilledButton(
                              ...
                              try {
                                ...
                                if (mounted) {
                                  final isCompletedNow = isChallenge
                                      ? (((_myProgress?['currentAmount']
                                                            as num?)
                                                        ?.toDouble() ??
                                                    0) +
                                                amount >=
                                            targetAmountVal)
                                      : (((_goal['current_amount'] as num?)
                                                        ?.toDouble() ??
                                                    0) +
                                                amount >=
                                            targetAmountVal);
                      ...
                      child: Text(
                        isChallenge ? 'Cập nhật tiến độ' : 'Đóng góp',
                      ),
                    ),
                  ),
                ],
  ...
  void _showEditGoalSheet() {
    ...
    final amountCtrl = TextEditingController(
      text: formatVnd(targetAmountVal).replaceAll('đ', '').trim(),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final isGroupGoal =
        typeStr == 'saving_group' ||
        typeStr == 'challenge_group' ||
        typeStr == 'group' ||
        _goal['is_group'] == true ||
        _goal['isGroup'] == true;
                ...
                children: [
                          ...
                          Tooltip(
                            ...
                            child: GestureDetector(
                              onTap: () {
                                ...
                                MimoSnackBar.showInfo(
                                  context,
                                  message: 'Đã sao chép mã mời: $inviteCode',
                                );
                              },
                              child: Container(
                                width: 36,
                          ...
                          children: [
                            ...
                            TextButton.icon(
                              onPressed: () {
                                ...
                                MimoSnackBar.showInfo(
                                  context,
                                  message: 'Đã sao chép mã mời vào bộ nhớ tạm!',
                                );
                              },
                          ...
                          if (!isCompleted)
                            SizedBox(
                              ...
                              child: FilledButton.icon(
                                ...
                                label: Text(
                                  isChallenge ? 'Đóng góp tiến độ' : 'Đóng góp',
                                ),
                                style: FilledButton.styleFrom(
                                  backgroundColor: AppColors.teal,
                    ...
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (ctx, i) =>
                              _buildChallengeLeaderboardItem(
                                context,
                                _progressLeaderboard[i],
                                i,
                                targetAmount,
                              ),
                        ),
                  ...
                  if (!isChallenge &&
                      (_topContributors.isNotEmpty ||
                          _contributorLeaderboard.isNotEmpty)) ...[
                    const SizedBox(height: 28),
                    Padding(
                      padding: const EdgeInsets.symmetric(
                        horizontal: AppSpacing.xxl,
                        ...
                        child: Container(
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (ctx, i) {
                              ...
                              final uName =
                                  m['username'] as String? ?? 'Thành viên';
                              final avatar = m['avatarUrl'] as String?;
                              final total =
                                  (m['totalContributed'] as num?)?.toInt() ??
                                  (m['total'] as num?)?.toInt() ??
                                  0;
                              final pct =
                                  (m['percentage'] as num?)?.toInt() ?? 0;
                      ...
                      child: Container(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            final uName =
                                c['username'] as String? ?? 'Thành viên';
  ...
  Widget _buildPodiumItem(BuildContext context, dynamic contributor, int rank) {
    ...
    final total =
        (contributor['totalContributed'] as num?)?.toInt() ??
        (contributor['total'] as num?)?.toInt() ??
        0;
  ...
  Widget _buildChallengeLeaderboardItem(
    ...
    final double progressRatio = targetAmt > 0
        ? (currentAmt / targetAmt).clamp(0.0, 1.0)
        : 0.0;
    final bool isCurrentUser =
        m['userId'] != null && m['userId'] == _currentUserId;
