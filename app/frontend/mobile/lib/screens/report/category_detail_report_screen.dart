...
class _CategoryDetailReportScreenState
  ...
  Future<void> _loadTransactions() async {
    ...
    try {
      ...
      try {
        final res = await AppQueries.transactions(
          widget.walletId,
          pageSize: 500,
        ).result;
      ...
      if (rawList.isEmpty) {
        try {
          final apiRes = await ApiClient().getTransactions(
      ...
      final targetCanonical = CategoryTheme.canonicalCodeOf(
        widget.categoryCode,
      ).toLowerCase();
      final targetLabel = CategoryTheme.of(
        widget.categoryCode,
      ).label.toLowerCase();
      ...
      final filtered = rawList.where((t) {
        ...
        final tCode =
            t['categoryCode']?.toString() ??
            t['category_code']?.toString() ??
            '';
        final tName =
            ...
            '';
        final isOther =
            targetCanonical == 'other' || targetCanonical == 'others';
        if (isOther) {
      ...
      if (mounted) {
        setState(() {
          ...
          if (_allItems.isNotEmpty) {
            ...
            final dateStr =
                first['occurredAt']?.toString() ??
                first['occurred_at']?.toString() ??
                '';
            if (dateStr.length >= 7) {
  ...
  List<dynamic> get _monthItems {
    return _allItems.where((t) {
      if (t is! Map<String, dynamic>) return false;
      final dateStr =
          t['occurredAt']?.toString() ?? t['occurred_at']?.toString() ?? '';
  ...
  List<Map<String, dynamic>> get _monthlyTrend {
    ...
    for (int i = 5; i >= 0; i--) {
      ...
      final total = monthItems.fold<int>(0, (sum, t) {
        return sum +
            (((t as Map<String, dynamic>)['amount'] as num?)?.toInt() ?? 0);
      });
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            Text(
                              'Các khoản ${widget.recordType == 'income' ? 'thu' : 'chi'}',
                              style: TextStyle(
                                color: context.palette.textPrimary,
                                fontSize: 16,
                                fontWeight: FontWeight.w700,
  ...
  Widget _buildTrendChart(List<Map<String, dynamic>> trend, Color color) {
    final maxVal = trend.fold<int>(
      0,
      (a, b) => (b['total'] as int) > a ? b['total'] as int : a,
    );
    final now = DateTime.now();
