...
Phân hệ này đảm nhận nhiệm vụ tiếp nhận và phân tích các câu văn bản tiếng Việt tự nhiên của người dùng, từ đó nhận diện ý định giao tiếp và bóc tách các tham số cần thiết để tự động ghi chép giao dịch tài chính, thực thi lệnh điều khiển hệ thống hoặc phản hồi hội thoại. Nhằm tối ưu hóa giữa tốc độ phản hồi tức thời và độ chính xác hiểu ngữ nghĩa phức tạp, phân hệ được thiết kế theo mô hình kiến trúc phân tầng chuyên biệt, phân luồng yêu cầu đầu vào qua hai cấp độ suy luận liên hoàn.
 
Hình 3.36: Sơ đồ luồng xử lý ngôn ngữ tự nhiên thực tế
Như minh họa tại Hình 3.36, quy trình vận hành của phân hệ xử lý ngôn ngữ tự nhiên được chia thành hai giai đoạn rõ rệt:
Tại Tầng 1, hệ thống thực hiện nhiệm vụ phân loại ý định của câu đầu vào để xác định người dùng đang muốn thực hiện thao tác nào trong ba nhóm chính: ghi chép thu chi (Record), lệnh điều khiển hệ thống (Action) hoặc trò chuyện xã giao (Chitchat). Tùy thuộc vào cấu hình do quản trị viên thiết lập, hệ thống có thể linh hoạt sử dụng mô hình học máy nhẹ TF-IDF, mô hình biểu diễn ngữ cảnh PhoBERT hoặc mô hình ngôn ngữ lớn Qwen 2.5. Kết quả phân loại tại tầng này quyết định chính xác nhánh xử lý chuyên sâu tiếp theo.
Tại Tầng 2, hệ thống điều phối yêu cầu theo từng luồng nghiệp vụ tương ứng:
- Nhóm ghi chép giao dịch (Record): Dữ liệu được đưa vào bộ bóc tách thực thể để nhận dạng các thông tin cốt lõi gồm số tiền, danh mục thu chi, loại bản ghi, ví tiền và ghi chú phát sinh. Quản trị viên có thể cấu hình sử dụng mô hình truyền thống (TF-IDF hoặc PhoBERT) để đạt tốc độ xử lý nhanh hoặc sử dụng mô hình ngôn ngữ lớn khi cần phân tích ngữ cảnh câu nói phức tạp.
- Nhóm lệnh điều khiển (Action) và hội thoại (Chitchat): Do yêu cầu khả năng hiểu cấu trúc câu ghép và ngữ pháp linh hoạt, các yêu cầu thuộc nhóm này được chuyển giao trực tiếp cho mô hình ngôn ngữ lớn Qwen 2.5 để trích xuất tham số điều khiển hoặc sinh câu trả lời tự nhiên.
Cách tổ chức phân luồng hai tầng này giúp giảm đáng kể không gian suy luận cho từng mô hình, hạn chế tối đa sự nhầm lẫn giữa các tác vụ nghiệp vụ và mang lại khả năng tùy biến linh hoạt giữa hiệu năng tốc độ và độ chuẩn xác của hệ thống.
