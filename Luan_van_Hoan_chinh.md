...
### 1.4. Đặc tả kịch bản Use Case chi tiết
Để làm rõ quá trình tương tác giữa Người dùng và hệ thống AI, dưới đây là đặc tả cho hai Use Case cốt lõi:

**Bảng 1.1. Đặc tả Use Case "Quét hóa đơn siêu tốc"**

| Thuộc tính | Mô tả |
|------------|-------|
| **Tên Use Case** | Quét hóa đơn bằng Camera |
| **Tác nhân** | Người dùng (User), Hệ thống xử lý AI (FastAPI OCR) |
| **Tiền điều kiện** | Người dùng đã đăng nhập và cấp quyền truy cập Camera. |
| **Luồng sự kiện chính** | 1. Người dùng chọn tính năng chụp ảnh hóa đơn.<br>2. Ứng dụng nén ảnh và tải lên máy chủ Cloud.<br>3. Backend nhận yêu cầu, gửi ảnh sang cụm AI (FastAPI) để xử lý bất đồng bộ, báo Client hiển thị trạng thái "Đang phân tích".<br>4. Hệ thống AI chạy mô hình VietOCR + LayoutLMv3 để trích xuất.<br>5. Backend cập nhật Database và đẩy dữ liệu qua WebSocket về Client.<br>6. Ứng dụng hiển thị Form điền sẵn dữ liệu, người dùng nhấn "Lưu". |
| **Luồng ngoại lệ** | - Hình ảnh quá mờ: AI trả về độ tin cậy thấp (<50%), ứng dụng yêu cầu nhập tay.<br>- Mất kết nối Internet: Lưu ảnh vào cache nội bộ, tự động thử lại khi có mạng. |
| **Hậu điều kiện** | Giao dịch mới được tạo thành công và số dư ví được cập nhật. |

**Bảng 1.2. Đặc tả Use Case "Giao tiếp với Trợ lý AI"**

| Thuộc tính | Mô tả |
|------------|-------|
| **Tên Use Case** | Chat với Trợ lý ảo (NLU) |
| **Tác nhân** | Người dùng (User), Trợ lý AI (NLU_FastAPI) |
| **Tiền điều kiện** | Người dùng đang ở màn hình Chat. |
| **Luồng sự kiện chính** | 1. Người dùng nhập văn bản hoặc nói qua Microphone (STT).<br>2. Backend chuyển câu nói sang AI Server.<br>3. PhoBERT phân loại Intent (Ý định) và gán nhãn Entity.<br>4. Nếu là `Record`: Backend tạo bản nháp giao dịch. Nếu là `Action`: Backend thực thi lệnh hệ thống.<br>5. Trợ lý AI sinh ra câu phản hồi phù hợp với "Tính cách" (Vibe) đã cấu hình và trả về màn hình Chat. |
