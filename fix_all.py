...
def fix_snackbars(content, filepath):
    ...
    # 1. Simple single line ones
    # ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('...'), backgroundColor: AppColors.danger));
    
    def replacer(match):
        is_const = match.group(1)
    ...
    # Regex for single line: ScaffoldMessenger.of(context).showSnackBar([const ]SnackBar(content: Text(...)[, ...]));
    pattern_single = re.compile(
        r'ScaffoldMessenger\.of\(context\)\.showSnackBar\((?:const\s+)?SnackBar\(\s*content:\s*Text\((.*?)\)(.*?)\)\s*\);',
        re.DOTALL
    )
